...
const ParticleSystem = ({
  ...
  }, [count, sourcePosition, targetPosition, containerBounds, type, getParticleColor]);
    ...
    setParticles(prevParticles => {
      return prevParticles.map(particle => {
        ...
        if (newParticle.life <= 0) {
          ...
          return {
            id: newId,
            x: (sourcePosition.x / 100) * containerBounds.width + (Math.random() - 0.5) * 40,
            y: (sourcePosition.y / 100) * containerBounds.height + (Math.random() - 0.5) * 40,
            targetX: (targetPosition.x / 100) * containerBounds.width + (Math.random() - 0.5) * 60,
            targetY: (targetPosition.y / 100) * containerBounds.height + (Math.random() - 0.5) * 60,
            velocity: {
        ...
        switch (type) {
          case 'connect':
            ...
            newParticle.x = particle.x + (particle.targetX - particle.x) * progressToTarget;
            newParticle.y = particle.y + (particle.targetY - particle.y) * progressToTarget;
  ...
  }, [isActive, intensity, type, sourcePosition, targetPosition, containerBounds, getParticleColor]);
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0">
        {type === 'float' && particles.slice(0, 5).map(particle => (
          <motion.div
            key={`css-${particle.id}`}
